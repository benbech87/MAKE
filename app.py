...
def _draw_rep(cv, rep, today, rds):
    ...
    for d in od:
        ...
        ann.append(dict(deal=d, task=t, lbl=lbl, sub=sub, col=col, pri=pri, amount=amt, days_late=dl, parked=pk))
    ...
    kpis = [(str(len(od)), 'OPEN DEALS'), (fmt_ccy(pv), 'PIPELINE VALUE'), (str(ov), 'OVERDUE'), (str(td), 'DUE TODAY'), (str(nt), 'NO TASK')]
...
def _sanitize_json_string_values(raw):
    out = []
    i = 0
    n = len(raw)
    in_string = False
    ...
    while i < n:
        ...
        if not in_string:
            ...
            continue
        if c == '\\':
            if i + 1 < n and raw[i + 1] in valid_escapes:
                out.append(c)
                out.append(raw[i + 1])
                i += 2
                continue
    ...
    try:
        ...
        if data is None:
            ...
            try:
                ...
                repaired = _sanitize_json_string_values(raw)
